...
window.addEventListener('scroll', function(){ // on page scroll
 requestAnimationFrame(parallax) // call parallax() on next available screen paint
		...
		/*========================================
		=        Color-changing "chari"          =
		========================================*/
		$(function(){
    // Check the initial Poistion of the chari element
    var chariScroll = $('#chari').offset().top;
    $(window).scroll(function(){
        if( $(window).scrollTop() > chariScroll%1.5 ) {
            //$('#sticky').css({position: 'fixed', top: '0px', float: 'right'});
            $('#chari').addClass("bgchange");
          $('#chari h1').css("color","rgb(0,0,0)");
        } else {
            $('#chari').removeClass("bgchange");
        }
    });
});
